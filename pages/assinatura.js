...
const ConfirmPage = () => {
  ...
  const handleAssinaturaChange = (e) => {
    setAssinaturaEletronica(e.target.value);
  };

  const handleRedirect = () => {
    if (assinaturaEletronica === '' || assinaturaEletronica.length < 8) {
      setError('A assinatura eletrônica deve ter pelo menos 8 caracteres.');
    } else {
      setError('');
